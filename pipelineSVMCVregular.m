...
k = 6; %k-mer
%**************
feature_indices_CV = intersect(feature_indices,size(features,2)-(4^k-1:-1:0)); %gapped features removed
%**************
% ------------------------------------------------------------
...
for repeatCV = 1:n_CVtests
    ...
    figure(11); hold on; plot(FPR,TPR);
    FPRs = [FPRs; FPR]; TPRs = [TPRs; TPR];
end
elapsedtime = toc;
mean(repeatAUC) 
title(sprintf('ROC. AUC_{avg} = %f in %u CV tests with SVM with contiguous %u-spectrum kernel.',mean(repeatAUC),n_CVtests,k));
ylabel('True positive rate'); xlabel('False positive rate'); box off;
% ------------------------------------------------------------
clear F; save(sprintf('tmpresultsSVMregular_CV%s.mat',testtime));
% ------------------------------------------------------------
